...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let end_to_end_id = url.searchParams.get('transfer_id') || url.searchParams.get('end_to_end_id');
    let company_id = url.searchParams.get('company_id');
    let transaction_id = url.searchParams.get('transaction_id');

    if (req.method === 'POST') {
      const body = await req.json();
      end_to_end_id = end_to_end_id || body.transfer_id || body.end_to_end_id;
    ...
    if (transaction_id && (!end_to_end_id || !company_id)) {
      const { data: txData } = await supabase
        .from('transactions').select('company_id, external_id, pix_e2eid')
    ...
    if (!company_id || !end_to_end_id) {
      return new Response(JSON.stringify({ error: 'company_id and end_to_end_id are required' }),
    ...
    try {
      const result = await callOnzProxy(
        `${baseUrl}/pix/payments/receipt/${encodeURIComponent(end_to_end_id)}`, 'GET',
        { 'Authorization': `Bearer ${access_token}` }
      );

      if (!result.ok) {
        return new Response(JSON.stringify({ error: 'Comprovante ainda não disponível. Tente novamente em alguns minutos.', details: result.data }),
          { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      // ONZ response: { data: { pdf: "base64string" } }
      const pdfBase64 = result.data?.data?.pdf || result.data?.pdf;
      if (!pdfBase64) {
        return new Response(JSON.stringify({ error: 'Comprovante não disponível na resposta' }),
          { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      return new Response(JSON.stringify({
        success: true,
        transfer_id: end_to_end_id,
        provider: 'onz',
