...
export default function Users() {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke("delete-user", {
        body: { user_id: deletingMember.user_id },
      });

      if (error) {
        toast({ variant: "destructive", title: "Erro", description: (error as any).message || "Falha ao excluir usu�rio." });
        return;
      }
  ...
  const handleResetPassword = async () => {
    ...
    if (newPassword.length < 6) {
      toast({ variant: "destructive", title: "Senha deve ter no m�nimo 6 caracteres" });
      return;
    }
    ...
    try {
      ...
      if (error) {
        toast({ variant: "destructive", title: "Erro", description: (error as any).message || "Falha ao atualizar senha." });
        return;
      }

      toast({ title: "Senha alterada com sucesso!", description: `A senha de ${passwordMember.profile?.full_name || "usu�rio"} foi atualizada.` });
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      if (error) {
        toast({ variant: "destructive", title: "Erro", description: (error as any).message || "Falha ao criar usu�rio." });
        return;
      }

      toast({ title: "Usu�rio cadastrado!", description: `${addName.trim()} foi criado e vinculado � empresa.` });
      setAddDialog(false);
      setAddEmail("");
      setAddName("");
      setAddPassword("");
